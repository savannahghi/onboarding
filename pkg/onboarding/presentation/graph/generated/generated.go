...
type ResolverRoot interface {
	...
	NavigationAction() NavigationActionResolver
	Query() QueryResolver
...
type NavigationActionResolver interface {
	OnTapRoute(ctx context.Context, obj *domain.NavigationAction) (string, error)
}
...
func (ec *executionContext) _NavigationAction_onTapRoute(ctx context.Context, field graphql.CollectedField, obj *domain.NavigationAction) (ret graphql.Marshaler) {
	...
	resTmp, err := ec.ResolverMiddleware(ctx, func(rctx context.Context) (interface{}, error) {
		ctx = rctx // use context from middleware stack in children
		return ec.resolvers.NavigationAction().OnTapRoute(rctx, obj)
	})
	if err != nil {
		ec.Error(ctx, err)
		return graphql.Null
	...
	for i, field := range fields {
		...
		case "onTapRoute":
			...
			out.Concurrently(i, func() (res graphql.Marshaler) {
				...
				res = ec._NavigationAction_onTapRoute(ctx, field, obj)
				if res == graphql.Null {
					atomic.AddUint32(&invalids, 1)
				}
				return res
