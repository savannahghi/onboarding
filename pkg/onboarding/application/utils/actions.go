...
// CheckUserHasPermission takes in the user roles and a permission an d verifies that the user
...
func GetUserNavigationActions(
	...
	//Get all navigation actions that the user has permission tom view
	allNavActions, err := domain.GetAllNavigationActions(ctx)
	if err != nil {
		return nil, err
	}
	//all user actions
	myNavigationActions := []domain.NavigationAction{}
	for _, action := range allNavActions {

		if action.RequiredPermission == nil ||
			CheckUserHasPermission(roles, *action.RequiredPermission) {
			//  check for favorite navigation actions
			if CheckUserHasFavNavAction(&user, action.Title) {
				action.Favorite = true
			}
			myNavigationActions = append(myNavigationActions, action)
		}
	}

	groupNested := GroupNested(myNavigationActions)
